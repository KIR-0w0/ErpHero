				...
				<div class="border p-2 bg-light" >
					<form id="form-estimates" method="POST" action="estimateform/insertEstimates" >
						<div class="mb-3">
							...
							<input type="text" class="form-control" id="client-vender" name="venderName" />
						</div>
						<div class="mb-3">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" id="client-business-number" name="businessNumber" />
						</div>
						<div class="mb-3">
							<label class="form-label">이메일</label>
							<input type="text" class="form-control" id="client-email" name="email" />
						</div>
						<div class="mb-3">
						...
						<div class="mb-5">
							...
							<button type="button" class="btn btn-primary btn-sm" id="btn-open-products-modal" >상품추가</button>
...
$(function() {
	...
	$("#form-estimates").submit(function(){
		var vender = $.trim($("#client-vender").val());
		if(!vender) {
		...
		if(!name) {
			alert("견적요청자는 필수 입력값입니다.")
			$("#client-name").focus();
			return false;
		}
		var number = $.trim($("#client-business-number").val());
		if(!number) {
			alert("사업자번호는 필수 입력값입니다.")
			$("#client-business-number").focus();
			return false;
		}
		var email = $.trim($("#client-email").val());
		if(!email) {
			alert("이메일은 필수 입력값입니다.")
			$("#client-email").focus();
			return false;
		}
		var phone = $.trim($("#client-phone").val());
		if(!phone) {
		...
		if(!deliveryDate) {
			alert("납기일은 필수 입력값입니다.")
			$("#delivery-date").focus();
			return false;
		}
		var productTr = $("#table-products-form tbody tr").length
		if(productTr == 0) {
			alert("상품을 추가해주세요.")
			return false;
		}
	...
	// 견적요청창에 상품추가 버튼 클릭시 발생하는 함수
	 $("#btn-open-products-modal").click(function() {
		var $tbody = $("#table-products-modal tbody").empty();
		$.ajax({
			type:"GET",
			url:"/estimateform/searchList",
			dataType:'json'
		})
		.done(function makeRow(product){
			$.each(product, function(index,product){
				...
				row += '<td class="text-center"><input type="number" name="estimateAmount" min="1" id="modal-amount"/></td>';
				row += "</tr>";
