...
$(function () {
	...
	var param = {};
	if ($("#start-date").val() == "") {
		param["startDate"] = new Date().getFullYear()+ "-" + ("0" + (new Date().getMonth())).slice(-2) + "-" + ("0" + new Date().getDate()).slice(-2);
		console.log($("#start-date").val());
	}
	if ($("#end-date").val() == "") {
		param["endDate"] = new Date().getFullYear()+ "-" + ("0" + (3 + new Date().getMonth())).slice(-2) + "-" + ("0" + new Date().getDate()).slice(-2);
...
function getAllData(param) {
	$.ajax({
		type:'post',
		url: '/dashboard/listInboundMonths/json',
	...
	$.ajax({
		type:'post',
		url: '/dashboard/listOrderMonths/json',
		data: JSON.stringify(param),	// 서버로 보내는 JSON 형식 데이터
		contentType: 'application/json',// 서버한테 json 형식 데이터가 보내진다고 알리는 거야
		dataType: 'json',		// 서버로부터 받은 응답이 json이어야돼
